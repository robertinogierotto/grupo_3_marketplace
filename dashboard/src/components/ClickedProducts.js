...
function ClickedProducts(product) {
    ...
    return (
        <div className="card mb-1 position-relative cardProducts" style={{ width: '18rem', height: '18rem' }}>
            <img className="card-img-top img-insideCard" src={`http://localhost:3001/images/products/${product.image}`} alt="Product" />
            ...
            <a href={`http://localhost:3001/admin/editProduct/${product.id}`} rel="noreferrer" target="_blank" className="btn btn-primary linkToDetail">Editar Producto</a>
        </div>
    );
}
